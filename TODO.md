...
> 最近更新: 2026-09-17 | 文档同步: v4.3.0 心智运行时
> 3.x 对标清单见 [docs/archive/演进方案-3x-冻结.md](docs/archive/演进方案-3x-冻结.md)（已归档，勿作路线图）
> **v4.3 → v5.0 演进总纲见 [演进方案.md](演进方案.md)**

## 当前版本: **4.3.0**（puax-mcp-server）

**P0–P2 已全部完成；P3 除日文版外已全部完成。v4.0 落地心跳 / 处境 / 自进化流水线；shaman- 八角色全留。**
...
- [x] **发散度量化 evals** — distinct-n / 语义半径 / 假设存活率（`evals/test-ghm-divergence.js`，无 LLM）
- [x] **幻觉泄漏检测** — 无印 / 免罪修辞强制 DISCARDED（源码门 + Jest）
- [x] **梦系离线对照** — 有梦协议禁止自动升格 vs 无梦锁定语（`evals/test-ghm-contrast.js`）
- [ ] L4 在线对照（真实模型梦系开 vs 关）— 需 API，不阻塞 4.0
...
| 版本 | 主题 | 核心交付 | 状态 |
|------|------|---------|------|
| **v4.0.0** | 心智运行时 | tick / AMP / 剧场 / TTF / Hook×7 / 指南心跳优先 | ✅ **当前** |
| **v3.12.0** | GHM 导引幻梦法 | 庄周八梦 + 三梦境工具 + 四触发 + 安全铁律机制化 | ✅ |
